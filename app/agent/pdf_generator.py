...
class PDFReportGenerator:
    ...
    def _add_page_number(self, canvas, doc):
        ...
        x_position = (letter[0] - text_width - canvas.stringWidth("Solvia", 'Helvetica-Bold', 10) - canvas.stringWidth(f" on {generation_date}", 'Helvetica', 10)) / 2
        ...
        canvas.setFillColor(SOLVIA_ORANGE)
        solvia_text = "Solvia"
        solvia_width = canvas.stringWidth(solvia_text, 'Helvetica-Bold', 10)
        canvas.drawString(x_position, 80, solvia_text)
        x_position += solvia_width

        # " on [date]" text
        canvas.setFont('Helvetica', 10)
        canvas.setFillColor(SOLVIA_GRAY)
        date_text = f" on {generation_date}"
        canvas.drawString(x_position, 80, date_text)
        ...
        quote_text = getattr(self, 'motivational_quote', '"Every journey begins with a single step."')

        # Remove quotes from the quote text for rendering
    ...
    def _create_page1_header(self, audit_data: Dict[str, Any], website_url: str, audit_id: str):
        ...
        <font color="{get_hex_string(SOLVIA_GRAY)}">Data Range:</font> {start_display} to {end_display}
        </para>
        """
    ...
    def _create_page1_summary(self, audit_data: Dict[str, Any]):
        """Create Page 1 summary section with 4 statements"""
        elements = []
        ...
        next_stage = seo_stage_info.get('next_stage', 'N/A')
        threshold_display = seo_stage_info.get('threshold_display', 'N/A')

        # Statement 1: Current visibility status
        statement1 = f"""
        <b>1. Visibility Status:</b> Your website is currently in the <font color="{get_hex_string(SOLVIA_ORANGE)}"><b>{stage_name}</b></font> stage
        with {current_impressions:,} impressions. You need {threshold_display} to reach the next stage.
        """

        # Statement 2: Traffic trend
        if isinstance(impressions_change, str):
            traffic_trend = "No historical data available"
            trend_color = SOLVIA_GRAY
        elif impressions_change > 0:
            traffic_trend = f"up {impressions_change:.1f}%"
            trend_color = SOLVIA_GREEN
        elif impressions_change < 0:
            traffic_trend = f"down {abs(impressions_change):.1f}%"
            trend_color = SOLVIA_RED
        else:
            traffic_trend = "stable"
            trend_color = SOLVIA_GRAY

        statement2 = f"""
        <b>2. Traffic Trend:</b> Over the last 28 days, your impressions are <font color="{get_hex_string(trend_color)}"><b>{traffic_trend}</b></font>,
        indicating {"growth" if isinstance(impressions_change, (int, float)) and impressions_change > 0 else "stability" if impressions_change == 0 else "decline" if isinstance(impressions_change, (int, float)) else "no change"}
        in search visibility.
        """

        # Statement 3: Click performance
        if isinstance(clicks_change, str):
            click_trend = "No historical data available"
            click_color = SOLVIA_GRAY
        elif clicks_change > 0:
            click_trend = f"increased by {clicks_change:.1f}%"
            click_color = SOLVIA_GREEN
        elif clicks_change < 0:
            click_trend = f"decreased by {abs(clicks_change):.1f}%"
            click_color = SOLVIA_RED
        else:
            click_trend = "remained stable"
            click_color = SOLVIA_GRAY

        statement3 = f"""
        <b>3. Click Performance:</b> Your total clicks have <font color="{get_hex_string(click_color)}"><b>{click_trend}</b></font>
        over the past 28 days, with a current total of {current_clicks:,} clicks.
        """

        # Statement 4: Position status
        if isinstance(position_change, str):
            position_trend = "No historical data available"
            position_color = SOLVIA_GRAY
        elif position_change < 0:  # Negative = improved (moved up in rankings)
            position_trend = f"improved by {abs(position_change):.1f} positions"
            position_color = SOLVIA_GREEN
        elif position_change > 0:  # Positive = declined (moved down in rankings)
            position_trend = f"declined by {position_change:.1f} positions"
            position_color = SOLVIA_RED
        else:
            position_trend = "remained stable"
            position_color = SOLVIA_GRAY

        statement4 = f"""
        <b>4. Ranking Position:</b> Your average position has <font color="{get_hex_string(position_color)}"><b>{position_trend}</b></font>,
        currently ranking at position {current_position:.1f}.
        """

        # Add all statements
        elements.append(Paragraph(statement1, self.styles['SolviaBody']))
        elements.append(Spacer(1, 8))
        elements.append(Paragraph(statement2, self.styles['SolviaBody']))
        elements.append(Spacer(1, 8))
        elements.append(Paragraph(statement3, self.styles['SolviaBody']))
        elements.append(Spacer(1, 8))
        elements.append(Paragraph(statement4, self.styles['SolviaBody']))
    ...
    def _create_page2_metrics_table(self, audit_data: Dict[str, Any]):
        ...
        elements.append(Paragraph("Key Performance Metrics", self.styles['SolviaHeading1']))
    ...
    def _create_page2_next_steps(self, audit_data: Dict[str, Any]):
        ...
        elements.append(Paragraph("Next Steps", self.styles['SolviaHeading2']))
