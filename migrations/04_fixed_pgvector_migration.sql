...
-- Main embeddings table for vector RAG
...
-- Vector similarity search index (only create if not exists)
DO $$
BEGIN
...
-- Insert default collections (safe with ON CONFLICT)
...
CREATE INDEX IF NOT EXISTS idx_keyword_documents_search ON keyword_documents USING GIN(search_vector);
...
-- =========================================
-- PHASE 6: CREATE POLICIES (HANDLE EXISTING)
-- =========================================

-- Drop existing policies if they exist, then recreate
DO $$
BEGIN
    -- Drop existing embeddings policies
    ...
    -- Drop existing keyword_documents policies
    DROP POLICY IF EXISTS "Users can view own keyword documents" ON keyword_documents;
    DROP POLICY IF EXISTS "Users can insert own keyword documents" ON keyword_documents;
    DROP POLICY IF EXISTS "Users can update own keyword documents" ON keyword_documents;
    DROP POLICY IF EXISTS "Users can delete own keyword documents" ON keyword_documents;
    
    -- Create keyword_documents policies
...
-- =========================================
-- PHASE 7: CREATE FUNCTIONS (REPLACE IF EXISTS)
-- =========================================
...
DROP TRIGGER IF EXISTS update_keyword_search_vector_trigger ON keyword_documents;
DROP TRIGGER IF EXISTS update_embeddings_timestamp ON embeddings;
...
-- =========================================
-- PHASE 8: SEARCH FUNCTIONS (REPLACE IF EXISTS)
-- =========================================
...
-- Keyword full-text search function
...
SELECT 
    ...
    CASE WHEN EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'embeddings') 
         THEN '✅ Created' 
         ELSE '❌ Missing' 
    END as status
...
SELECT 
    'keyword_documents table' as component,
    CASE WHEN EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'keyword_documents') 
         THEN '✅ Created' 
         ELSE '❌ Missing' 
    END as status
...
SELECT 
    'embedding_collections table' as component,
    CASE WHEN EXISTS (SELECT * FROM information_schema.tables WHERE table_name = 'embedding_collections') 
         THEN '✅ Created' 
         ELSE '❌ Missing' 
    END as status
...
SELECT 
    '🎉 FIXED PGVECTOR MIGRATION' as component,
